...
import { addItem, statesOfCart } from '../../redux/slices/cartSlice';

function PizzaBlock({ id, title, price, imageUrl, sizes, types }) {
  const typeNames = ['тонкое', 'традиционное'];

  const { count } =
    useSelector(statesOfCart).items.find((pizzaObject) => pizzaObject.id === id) || {count: 0};
        ...
        <div className="pizza-block__selector">
          <ul>
            {types.map((type, i) => (
              <li
                key={i}
                className={i === activeIndexOfType ? 'active' : ''}
          ...
          <ul>
            {sizes.map((size, i) => (
              <li
                key={i}
                className={i === activeIndexOfSize ? 'active' : ''}
